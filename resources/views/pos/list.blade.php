...
<style>
    ...
    .pos-container .tabbar {
        margin-top: 3px;
    }

    .pos-container .panel-invoice {
        min-height: 100%;
    }

    .pos-container .panel-invoice .calculator-roll li:nth-child(even) {
        background: #f0f4c3
    }

    .pos-container .panel-invoice .calculator-item {
        border: 1px solid trasparent;
        padding: 0px;
    }

    .pos-container .panel-invoice .calculator-item:hover {
        border: 1px solid silver;
        border-radius: 5px;
    }

    .pos-container .panel-invoice .calculator-roll {
        min-height: 600px;
    }
</style>
                ...
                <div class="col-md-8">
                    <div class="input-group mb-3">
                        <div class="input-group-prepend">
                            <span class="input-group-text" id="inputGroup-sizing-default"><i class="fa fa-barcode"
                                    aria-hidden="true"></i></span>
                        </div>
                        <input type="text" class="form-control searchProduct" name="search_product"
                            placeholder="Product with Name" aria-label="Default"
                        ...
                        <ul class="list-unstyled">
                            @foreach ($list['data'] as $key => $value)
                                ...
                                <li class="item" style="background-color: {{ $color }};">
                                    <a href="#" onclick="getStockItem({{ $value['id'] }})"
                                        value="{{ $value['price'] }}">
                                        <div class="text-center p-3 text-white">{{ $value['title'] }}</div>
                                    </a>
                                </li>
                            @endforeach
                        </ul>
                                ...
                                <table class="table">
                                    ...
                                    <tfoot class="billAmount">
                                        <tr>
                                            ...
                                            <td><input type="text" name="total_bill" class="form-control total_bill"
                                                    readonly style="border: none"></td>
                                        </tr>
                                        <tr>
                                            <th scope="row">Discount</th>
                                            <td><input type="text" name="discount" class="form-control discount">
                                            </td>
                                        </tr>
                                        <tr>
                                            <th scope="row">Payable Amount</th>
                                            <td><input type="text" name="payable_amount"
                                                    class="form-control payable_amount" readonly style="border: none">
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
            ...
            success: function(response) {
                if (response.qty < 0 || response.qty == 0) {
                    ...
                    var tr = "<tr><td>" + response.title + "</td>" +
                        "<td><input type='hidden' name='item[]' class='form-control item' value='" +
                        response.id +
                        "'><input type='number' name='qty[]' min='1' class='form-control qty' required></td>" +
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(response, function(key, val) {
                    output += '<li class="list-group-item selectSearchCustomer">' + val
                        .name + '</li>';
                });
                output += '</ul>';
                $('.display').html(output);
    ...
    $('.searchProduct').on('keyup', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(response, function(key, val) {
                    output +=
                        '<li class="list-group-item selectSearchItems" onclick="getStockItem(' +
                        val.id + ')">' + val.title + '</li>';
                });
                output += '</ul>';
